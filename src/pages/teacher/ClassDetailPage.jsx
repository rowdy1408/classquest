...
function ClassModal({ modal, setModal, classId, students, roleCatalog, app, editingNode, editingStudent, klass, tests }) {
  ...
  if (modal === 'student') return <StudentFormModal open onClose={close} editingStudent={editingStudent} students={app.data.students} roleCatalog={roleCatalog} onSubmit={async (form) => { const level = Math.max(1, Math.min(MAX_CHARACTER_LEVEL, Number(form.level) || 1)); const payload = { name: form.name, email: form.email, username: form.username, password: form.password, role: form.role, gender: form.gender, level, note: form.note || '', ...(!editingStudent ? { xpToNext: Math.max(100, level * 100) } : {}) }; if (editingStudent) { const result = await app.updateStudent(editingStudent.id, payload); if (result.ok) close(); return result; } const result = await app.addStudent({ ...payload, classIds: [classId] }); if (result.ok) close(); return result; }} />;

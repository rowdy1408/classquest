  ...
  return (
    <>
      <div className="page-toolbar">
        <div><h2>Thư viện quy tắc thưởng phạt</h2><p>Mỗi lớp mới có sẵn 8 quy tắc mẫu. Giáo viên có thể xóa, điều chỉnh điểm hoặc thêm quy tắc riêng.</p></div>
        <button className="button primary" disabled={!classes.length} onClick={openCreate}><Plus size={17} /> Thêm quy tắc</button>
      </div>
      <div className="rule-grid">
        {rules.map((rule) => {
          const klass = classes.find((item) => item.id === rule.classId);
          return (
      ...
      <Modal open={open} title="Thêm quy tắc" onClose={() => setOpen(false)}>
        <form className="form-grid" onSubmit={submit}>
          ...
          <label><span>Tên quy tắc</span><input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required /></label>
